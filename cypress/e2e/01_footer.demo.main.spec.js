  ...
  urlsToTest.forEach((urlsToTest) => {
    ...
    context(`Testing on URL: ${urlsToTest}`, () => {
      ...
      it('footer test cases', () => {
        ...
        cy.then(() => {
          // Object.keys(subMenuDetails).forEach((subMenu) => {
          //   if (!user.accordionData[subMenu]) {
          //     console.log(`SubMenu ${JSON.stringify(subMenuDetails[subMenu])} is present in subMenuDetails but not in userAccordionData`);
          //   }
          // });
