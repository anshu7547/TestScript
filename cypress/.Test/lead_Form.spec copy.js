  ...
  urlsToTest.forEach((urlsToTest) => {
    ...
    context(`Testing on URL: ${urlsToTest}`, () => {
      ...
      it('lead form', () => {
        cy.url().then((url) => {
          const getURL = url;
          if (getURL.includes('/open-demat-account') || getURL.includes('/en/knowledge-base/') || getURL.includes('/intraday-stocks')|| getURL.includes('stock-market-holidays')) {
            cy.get('.form-footer').should('exist');
            cy.get('.form-footer a')
      ...
      it('sticky footer', () => {
        ...
        cy.url().then((url) => {
          const getURL = url;
          if (getURL.includes('/en/knowledge-base/')) {
