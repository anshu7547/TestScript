    ...
    context(`Testing on URL: ${urlsToTest}`, () => {
      ...
      it('lead form', () => {
        cy.url().then((url) => {
          ...
          //   else if (getURL.includes('/en/knowledge-base/') && getURL.split('/').length === 6) {
          //     // Your logic for the case where the URL is greater than /en/knowledge-base/ and contains multiple slashes
          //     cy.get('.form-footer').should('exist');
          //     cy.get('.form-footer a').should('have.attr', 'href', '/assets/pdf/Final-terms-for-DIY-account-opening.pdf').and('have.attr', 'target', '_blank');
